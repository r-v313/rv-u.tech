...
function escapeHTML(str) {
  ...
  (function () {
    ...
    // Force the cinematic preloader to play even when Windows/Chromium reports
    // prefers-reduced-motion. Only the preloader is opted back into motion; the
    // rest of the page keeps its existing reduced-motion accessibility rules.
    //
    // We insert the override into the already-authorized same-origin stylesheet
    // through CSSOM instead of creating an inline <style>, because this page's
    // CSP intentionally blocks inline styles.
    preloader.classList.add('rvu-force-motion');
    (function installIntroMotionOverrides() {
      const rules = [
        '#preloader.rvu-force-motion{transition:clip-path .78s cubic-bezier(.76,0,.24,1),opacity .22s ease,visibility .22s ease!important}',
        '#preloader.rvu-force-motion .space-canvas,#preloader.rvu-force-motion .intro-scanline,#preloader.rvu-force-motion .intro-analysis-words,#preloader.rvu-force-motion .intro-grid{display:block!important}',
        '#preloader.rvu-force-motion .preloader-content{opacity:0!important;transform:scale(.82)!important;filter:blur(11px)!important;transition:none!important}',
        '#preloader.rvu-force-motion.stage-brand .preloader-content,#preloader.rvu-force-motion.stage-outro .preloader-content{opacity:1!important;transform:scale(1)!important;filter:blur(0)!important;transition:opacity .72s cubic-bezier(.16,1,.3,1),transform .88s cubic-bezier(.16,1,.3,1),filter .72s ease!important}',
        '#preloader.rvu-force-motion.stage-outro .preloader-content{opacity:0!important;transform:scale(1.42)!important;filter:blur(5px)!important;transition-duration:.72s!important}'
      ];
      ...
      for (const sheet of Array.from(document.styleSheets)) {
        try {
          ...
          if (sheet.href) {
            ...
            if (location.protocol !== 'file:' && url.origin !== location.origin) continue;
            if (/styles\.css(?:$|[?#])/.test(url.href)) { targetSheet = sheet; break; }
          } else if (!targetSheet) {
            targetSheet = sheet;
          }
        } catch (_) {}
      ...
      if (!targetSheet) {
        for (const sheet of Array.from(document.styleSheets)) {
          try { if (sheet.cssRules) { targetSheet = sheet; break; } } catch (_) {}
        }
      }

      if (targetSheet) {
        for (const rule of rules) {
          try { targetSheet.insertRule(rule, targetSheet.cssRules.length); } catch (_) {}
        }
      }
    })();

    // Do not use the OS reduced-motion preference to short-circuit this intro.
